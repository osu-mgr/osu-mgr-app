...
import { searchByType, searchByUUIDs, countByUUIDs } from '../es';
import { Account, Cruise, Core, Section, SectionHalf } from '../stores/items';
...
^FH^FDLA,https://osu-mgr.org/${data.osuid}^FS
^FO357,155^BQN,2,4
^FH^FDLA,IGSN: ${data.osuid}
Lat: ${data.latStart}${data.latEnd}
Long: ${data.lonStart}${data.lonEnd}
Depth: ${data.depthStart}${data.depthEnd}
Total Sections in Core: ${data.nSections}
Total Length: ${data.coreLength}
Section Length: ${data.sectionLength}
Top: ${data.sectionTop}
Bottom: ${data.sectionBottom}
PI: ${data.pi}^FS
...
const ItemsPrintLabelsModal: FunctionComponent<{
  ...
  useEffect(() => {
    if (
      ...
      (async () => {
        setSectionHalves(
          (await searchByUUIDs(uuids, 0, 10000, 'sectionHalf')) as SectionHalf[]
        );
      })();
  }, [uuids, sectionHalves, sectionHalvesCount, isOpen]);
  const [nLabels, setNLabels] = useState<number>(1);
  useEffect(() => {
    if (isOpen && uuids !== undefined)
      (async () => {
        setSectionHalvesCount(await countByUUIDs(uuids, 'sectionHalf'));
      })();
  }, [uuids, isOpen]);
  useEffect(() => {
    if (isOpen && cruises === undefined && sectionHalves !== undefined)
      (async () => {
        ...
        setCruises(cruisesByUUID);
      })();
  }, [uuids, sectionHalves, isOpen, cruises]);
  useEffect(() => {
    if (isOpen && cores === undefined && sectionHalves !== undefined)
      (async () => {
        ...
        setCores(coresByUUID);
      })();
  }, [uuids, sectionHalves, isOpen, cores]);
  useEffect(() => {
    if (isOpen && sections === undefined && sectionHalves !== undefined)
      (async () => {
        ...
        setSections(sectionsByUUID);
      })();
  }, [uuids, sectionHalves, isOpen, sections]);
  useEffect(() => {
    if (
      isOpen &&
