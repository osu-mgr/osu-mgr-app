...
import { Cruise, itemTypesIcon } from '../stores/items';
...
const ItemsCruiseProgramModal: FunctionComponent<{
  ...
  useEffect(() => {
    if (isOpen && uuid !== undefined)
      (async () => {
        ...
        if (isMounted()) setCruise(update as Cruise);
      })();
  ...
  useEffect(() => {
    if (
      ...
      (async () => {
        const update = await countByUUIDs([cruise._cruiseUUID || ''], 'core');
        if (isMounted()) setCoresCount(update);
      })();
  }, [isMounted, uuid, isOpen, cruise]);
  useEffect(() => {
    if (
      isOpen &&
      uuid !== undefined &&
      ...
      (async () => {
        const update = await countByUUIDs([cruise._cruiseUUID || ''], 'dive');
        if (isMounted()) setDivesCount(update);
      })();
  }, [isMounted, uuid, isOpen, cruise]);
  return (
    <Modal
      trigger={children}
      icon={itemTypesIcon.cruise}
